...
function initSounds() {
  // Create oscillators but do NOT start them yet. Browsers require a user
  // gesture to unlock/resume the Web Audio context; we'll start them after
  // the first user interaction (click/keypress) via `ensureAudioUnlocked()`.
  laserOsc = new p5.Oscillator('sawtooth');
  laserOsc.freq(1200);
  laserOsc.amp(0);
...
function startOscillators() {
  ...
  if (laserOsc && typeof laserOsc.start === 'function') laserOsc.start();
  if (hitOsc && typeof hitOsc.start === 'function') hitOsc.start();
  oscillatorsStarted = true;
}

function ensureAudioUnlocked() {
  if (audioUnlocked) return Promise.resolve();
  // p5 provides `userStartAudio()` which should be called from a user gesture
  if (typeof userStartAudio === 'function') {
    return userStartAudio().then(() => {
      audioUnlocked = true;
      startOscillators();
    }).catch(() => {
      // fallback: try to resume the AudioContext
  ...
  // fallback if userStartAudio is not available
  const ac = (typeof getAudioContext === 'function') ? getAudioContext() : (window.AudioContext && new AudioContext());
  if (ac && ac.state === 'suspended' && ac.resume) {
    return ac.resume().then(() => {
      audioUnlocked = true;
...
function playLaser() {
  if (!laserOsc) return;
  const freq = 900 + Math.random() * 800;
  laserOsc.freq(freq);
  laserOsc.amp(0.28, 0.005);
  setTimeout(() => { laserOsc.amp(0, 0.12); }, 50);
}
...
function playHit() {
  ...
  hitOsc.freq(freq);
  hitOsc.amp(0.18, 0.005);
  setTimeout(() => { hitOsc.amp(0, 0.18); }, 60);
}
